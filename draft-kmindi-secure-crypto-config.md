...
For signing of the corresponding JSON files openssl could be used. 
...
The check for valid signature of each Secure Crypto Config file should be made before the actual start of the program to guarantee that only non forged Secure Crypto files are used during execution.
...
The delivery of the Secure Crypto Config will be done within a subfolder inside the Interface. Therefore, with each version of the Interface the current and the previously supported Secure Crypto Config will be provided. The Secure Crypto Config will be stored inside a subfolder `configs` inside its `src`-folder. Insode this subfolder they are organized in the same hierarchy as inside its official repository (see {{RepoFormat}}). In each new version of the Interface the newest Secure Crypto Configs will be added. If new Secure Crypto Configs will be published which are not already contained inside the corresponding Interface version they must be added manually.
